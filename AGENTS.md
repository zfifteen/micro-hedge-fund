...
- Daily session reads two dedicated Robinhood lists only: `+MHF-candidates` (active names under consideration; presence is not a buy) and `+MHF-macro` (index/factor proxies for market context). Do not treat other account watchlists as fund input. Cap `+MHF-candidates`. Remove a name that has not earned a sentence in two sessions.
