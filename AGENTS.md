...
- Two convenience Robinhood lists exist for research speed: `+MHF-candidates` and `+MHF-macro`. They do not limit the scope of session reasoning. The agent may consider any name, factor, or source it judges relevant. The agent may add to, remove from, rename, or otherwise modify these lists as it sees fit. Presence on a list is not a buy. The lists are a scratchpad, not a universe.
